...
while True:
    for comment in subreddit.stream.comments():
        try:
            progress()
            if findt(comment.body, 1) and wordmatch(str(comment.body).lower(), 'plagueis') and wordmatch(str(comment.body).lower(), 'tragedy') and not str(comment) in Cache.get('actions') and not findmatch(comment.body, Tragedy, 66, 1):
                print('')
                print(comment)
                print(comment.body)
